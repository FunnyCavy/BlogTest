...
为了和 `material-theme` 的主题搭配，同步更改了代码块相关的其他样式，VitePress 定义了很多 CSS 变量，通过简单的修改这些[内置的变量](https://github.com/vuejs/vitepress/blob/main/src/client/theme-default/styles/vars.css)即可方便的自定义样式：
...
扩展默认主题，在页面加载或路由变化的时候初始化 `mediumZoom`，使其应用到 `.main img`，也就是文档中的所有图片，然后设置背景颜色和间距
